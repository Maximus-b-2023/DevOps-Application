...
def test_join_group(client):
    ...
    society = Societies(
        name="test_group", description="Test description", created_by=creator.staff_id
    )
...
def test_non_admin_edit_group(client):
    ...
    society = Societies(
        name="test_group", description="Test description", created_by=staff.staff_id
    )
    db.session.add(society)
    db.session.commit()
...
def test_edit_account(client):
    ...
    update_account = {
        "staff_username": "edited_username",
        "staff_email": "edited_email@email.com",
        "password": "",
        "confirm_password": "",
    }
...
def test_date_availabilty(client):
    ...
    response = client.post(
        f"/group/{staff.staff_id}", data=new_dates, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_announcement(client):
    ...
    response = client.post(
        f"/group/{society.society_id}/announcement",
        data=update_announcement,
        follow_redirects=True,
    )
    assert response.status_code == 200
